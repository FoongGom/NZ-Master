...
목적:
- 층간소음을 여러 유형으로 가상 생성한다.
- 각 소음 유형에 대해 여러 DSP 제어 방식을 비교한다.
- 최종적으로 소음 유형에 따라 가장 적절한 제어 방식을 선택한다.
...
2. FxNLMS Adaptive
   - FxNLMS 적응형 제어
   - 스피커/판 전달 경로를 고려한 적응형 제어 방식

3. Ringdown Impact Control
   - 충격 잔향 저감 제어
   - 순간 충격 이후 남는 저주파 잔향을 줄이는 방식
...
# 샘플링 주파수
# fs = 1000이면 1초에 1000개의 데이터를 만든다는 뜻
fs = 1000
...
# 3. 실제 출력 경로 모델
# =========================================================
# 시뮬레이션을 현실적으로 만들기 위해 스피커/앰프/판 전달 경로를 단순 모델로 추가함.
#
# 실제 시스템:
# 라즈베리파이 출력
# → 앰프
...
#
# 이 과정에서 지연과 왜곡이 발생함.
# 이것을 secondary path라고 부름.

def apply_secondary_path(signal, secondary_path):
    """
    제어 신호가 스피커/판 전달 경로를 통과한 결과를 계산.

    기존 단순 시뮬레이션:
    원래 신호 + 제어 신호

    현실 반영 시뮬레이션:
    원래 신호 + 전달 경로를 통과한 제어 신호
    """
...
def make_secondary_path(delay_samples=8, length=64):
    """
    단순화된 secondary path 생성.

    delay_samples:
    - 출력 신호가 센서 위치에 도달하기까지의 지연
...
print()
print("=" * 160)
print("전체 실험 결과 요약 - 소음 유형별 하이브리드 제어")
print("=" * 160)
print(
    "소음 종류".ljust(34),
    "| 분류".ljust(30),
...
plt.title("Hybrid Control Result by Noise Type (소음 유형별 하이브리드 제어 결과)")
plt.legend()
plt.grid(axis="y")
plt.tight_layout()
plt.show()